...
Invalid package IDs, object IDs, coin metadata, or partner addresses throw `TypeError` during client setup. Resolving a coin with missing price-info object configuration also throws `TypeError`. Non-string partner values passed from JavaScript also throw `TypeError` with an `Invalid partner address configuration` message.
